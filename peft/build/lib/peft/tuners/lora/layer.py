...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        self.lora_C = torch.eye(self.lora_B[adapter_name].weight.size(0),dtype=self.lora_B[adapter_name].weight.dtype, device=self.lora_B[adapter_name].weight.device)
        if use_rslora:
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            if self.lora_C is None:
                lora_output = lora_B(lora_A(dropout(sub_batch))) * scaling
            else:
                lora_output = (lora_B(lora_A(dropout(sub_batch))) * scaling) @ self.lora_C.T.to("cuda")
...
class Linear(nn.Module, LoraLayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        if self.lora_C is not None:
            weight_C = self.lora_C.to(weight_B.device)
        else:
            weight_C = torch.eye(weight_B.size(0)).to(weight_B.device)
        ...
        output_tensor = transpose(weight_C @ weight_B @ weight_A, self.fan_in_fan_out) * self.scaling[adapter]
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                lora_A = self.lora_A[active_adapter]
                lora_B = self.lora_B[active_adapter]
                lora_C = self.lora_C.to(lora_B.weight.device)
                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                ...
                if not self.use_dora[active_adapter]:
                    if lora_C is None:
                        ...
                        result = result + (lora_B(lora_A(dropout(x))) * scaling) @ lora_C.T.to(x.device)
                else:
                    x = dropout(x)
